...
const PosterGhost = ({ className }: { className?: string }) => (
  <div
    aria-hidden
    className={cn(
      'aspect-2/3 w-16 rounded-lg border border-white/10 bg-linear-to-b from-muted/90 to-muted/20 shadow-lg sm:w-20',
      className
    )}
  />
)
...
export function EmptyState({
  ...
  return (
    ...
    >
      ...
      <div
        style={riseDelay(0)}
        className="relative mb-8 flex animate-rise-in items-end justify-center"
      >
        <div
          aria-hidden
          className="absolute top-1/2 left-1/2 size-36 -translate-1/2 rounded-full bg-primary/20 blur-3xl"
        />
        ...
        <div className="absolute top-1/2 left-1/2 z-20 grid size-16 -translate-1/2 place-items-center rounded-2xl border border-cyan-300/30 bg-background/70 text-cyan-300 shadow-xl ring-1 ring-primary/20 backdrop-blur-md">
      ...
      <Heading
        style={riseDelay(1)}
        className="animate-rise-in text-xl font-semibold text-balance text-foreground sm:text-2xl"
      >
        {title}
      </Heading>
      <p
        style={riseDelay(2)}
        className="mt-2 max-w-sm animate-rise-in text-sm leading-relaxed text-pretty text-muted-foreground sm:text-base"
      >
        {description}
      </p>
