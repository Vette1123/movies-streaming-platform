...
bundle, then resolve the stream. Only the first link can be paid for _before_
...
- **Extracted the cache with a throwaway script that only did half the job.**
  ...
  `git diff --stat` for the _matching_ insert before trusting the delete.
...
1. Anything that can be fetched before the tap should be — but cache the
   _promise_, and expire it well inside the lifetime of what it holds.
