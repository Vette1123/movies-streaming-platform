...
- **"Export my data" was proposed as a new supporter feature.** It has shipped since the accounts work: `DataPanel` builds the whole library into a JSON file in the browser, and it is deliberately free — a page that paywalls the exit is one nobody trusts with anything. Reading the panel before proposing the feature would have cost thirty seconds; the shortlist was written from memory of what the console _felt_ like it had.
- **`whenLabel` called anything 7 to 13 days out "Next week".** A fortnight's worth of dates wearing one label, and thirteen days is not next week by anybody's reckoning. It says `Next Tuesday` now, which is shorter and true.
...
- **Collect a field while the response is already in hand.** `watched_media.runtime` (migration `0003`) is written by the sweep and read by nothing. It costs one more field off a response that was going to be discarded; asking for it later would be one TMDB request per title in somebody's library.
- **Read the panel before proposing to build what is in it.**
