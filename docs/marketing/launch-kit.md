...
**The rule covers assets, not just text.** The OG card
(`app/_og/source.tsx` → `public/opengraph-image.png`) is the single most-shared
piece of copy the product has: it is what every unfurl renders, and it was the
first gallery image on the Product Hunt draft. Until 31 Aug 2026 it read "FREE
TO STREAM / Watch movies & TV shows. Free. … no signup, no paywall" — the exact
framing the paragraph above forbids, shipping on every share while the written
copy stayed careful. It now reads "OPEN SOURCE / Find what to watch, then keep
track." Re-read the image, not just the file, after `pnpm og:build` (and run
`pnpm fonts:og` first — the glyph subset in `app/_fonts/load.ts` is built from
that exact string, so new characters render as blanks otherwise).
...
- **RunaCapital/awesome-oss-alternatives** (19.6k ★) — the criteria require "a
  private for-profit company, founded in the last 10 years". Reely is not a
  company; there is no reading of that rule it passes.
- **tortuvshin/open-apps** / openappscout.com (4.3k ★, actively curated) — the
  `stack` taxonomy is mobile-only (`flutter`, `react-native`, `ios`, `android`,
  `capacitor`, `kmp`, `tauri`). A web app has no legal value there, and their
  CONTRIBUTING says not to add a taxonomy value for a single project.
- **Axorax/awesome-free-apps** (7.6k ★) — desktop and mobile apps only; every
  entry carries 🪟/🍎/🐧 platform icons and the nearest category is video
  editors and players. A PWA does not fit the shape of the list.
- **irazasyed/awesome-cloudflare** (1.2k ★) — has no section for apps built on
  Workers. "Recipes" is a list of worker scripts you copy, mostly from 2019;
  Reely's Worker is not a recipe.
- **unicodeveloper/awesome-opensource-apps** (3.9k ★) — looked like the perfect
  fit by its description, but the README in the repo today is an unrelated
  "Awesome Python Scripts" list. Nothing to submit to.
- **Igglybuff/awesome-piracy** (26.9k ★) — the single biggest list this app
  would "fit". Do not. One row there redefines the product for every moderator,
  processor and reviewer who searches the name afterwards.
...
| **Product Hunt**                       | Scheduled: launch Tue 8 Sep 2026, 12:01am PT. Gallery, tagline, description, topics and the maker comment are on the draft; the date is still editable from the launch page. Deliberately a week after the downloader launch so the two do not stack. **31 Aug:** the first gallery image (which is also the social preview when the PH link is shared) was the old OG card reading "FREE TO STREAM / Watch movies & TV shows. Free. … no signup, no paywall" — replaced with the rewritten card, old one deleted. Three shoutouts added (Cloudflare Workers, Next.js, shadcn/ui), because PH says launches with shoutouts are likelier to be featured and each one also leaves a founder review on that product's page.                                                                                        |
...
| **Clone-Wars** (36.7k ★)               | PR open: https://github.com/GorvGoyl/Clone-Wars/pull/303 — "Clones and Alternatives" table, one row in alphabetical position after the existing IMDB row, listed as an **IMDB / JustWatch** alternative (never Netflix — that reads as a catalog, not a catalogue). Biggest list Reely legitimately belongs in, but the repo has not been pushed since Aug 2024 and the maintainer has an open issue asking someone to take over merging PRs, so do not expect it to land.                                                                                                                                                                                                                                                                                                                                      |
| **awesome-no-login-web-apps** (3.3k ★) | PR open: https://github.com/aviaryan/awesome-no-login-web-apps/pull/571 — bottom of **Miscellaneous**, per their "add at the bottom of the category" rule, with the PR template's three checkboxes filled. Reely qualifies without the `[Account]` tag: the watchlist, history and searches are all local, and the Google sign-in is optional sync only. List is stale (last push Apr 2024).                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **OpenAlternative**                    | Submitted (free queue): https://openalternative.co/submit/reely, as an alternative to **IMDb**. Worth it because the approved listing is auto-published into their awesome-list repo (`piotrkulpinski/open-source-alternatives`, 6.7k ★) — one form, two placements. The paid tiers ($97 / $137 / $197) only skip the review queue; declined.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
