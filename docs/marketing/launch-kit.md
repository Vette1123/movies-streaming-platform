...
| **Google Search Console** | Already verified as `sc-domain:reely.space`, sitemap read 30 Aug: **14,373 URLs discovered, 30.9k pages indexed, 48.4k not**. The biggest bucket is 15,962 `Server error (5xx)` — the ghost of the OpenNext CPU kills, now in "validation started". Worth watching; it is SEO work, not marketing. |
