...
| **Hacker News** | Not attempted: the browser extension refuses to navigate to news.ycombinator.com. Show HN, with the static-export migration as the lede, is still the single highest-value thing left. |
| **Reddit** | Deliberately skipped — the account is under a sitewide spam filter (see the downloader lesson). |
| **Everything else** | Not attempted. SaaSHub, Indie Hackers, StartupBase and Uneed are all forms; fill them from the copy above. |
